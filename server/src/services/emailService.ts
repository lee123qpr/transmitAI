...
export const sendSevenDayFollowUp = async (email: string): Promise<boolean> => {
    if (!process.env.RESEND_API_KEY) return false;
    ...
    const template = await getEmailTemplate('email_template_seven_day_followup', 'Checking in - Transmit AI', defaultHtml);
...
export const sendTrialGifted = async (email: string): Promise<boolean> => {
    if (!process.env.RESEND_API_KEY) return false;
    ...
    const template = await getEmailTemplate('email_template_trial_gifted', 'Enjoy 14 Days of Pro for Free!', defaultHtml);
...
export const sendTrialEnded = async (email: string): Promise<boolean> => {
    if (!process.env.RESEND_API_KEY) return false;
    ...
    const template = await getEmailTemplate('email_template_trial_ended', 'Your Transmit AI Pro Trial has ended', defaultHtml);
