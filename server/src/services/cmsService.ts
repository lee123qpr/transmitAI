...
export const upsertArticle = async (article: Partial<Article>, adminId: string): Promise<Article> => {
    ...
    if (id) {
        const res = await query(
            ...
            [title, slug, content, excerpt, published, header_image, keywords, id]
        );
        ...
        const res = await query(
            ...
            [title, slug, content, excerpt, published, header_image, keywords, adminId]
        );
...
export const upsertAnnouncement = async (announcement: Partial<Announcement>, adminId: string): Promise<Announcement> => {
    ...
    if (id) {
        const res = await query(
            ...
            [message, type, link, active, id]
        );
        // If this one is activated, maybe deactivate others? (Optionally)
